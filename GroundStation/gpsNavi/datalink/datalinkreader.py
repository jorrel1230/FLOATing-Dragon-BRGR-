...
ser = serial.Serial('/dev/tty.usbserial-D30EZWU7', 230400, timeout=0.1)
...
while True:
    ...
    if validData is not None and len(validData) > 0:
        ...
        match dataType:
            case 'GPS LAT':
                gpsDataArr[0] = processed_data
            case 'GPS LON':
                gpsDataArr[1] = processed_data
            case 'GPS ALT':
                gpsDataArr[2] = processed_data
            case 'IMU ACX':
                accDataArr[0] = processed_data
            case 'IMU ACY':
                accDataArr[1] = processed_data
            case 'IMU ACZ':
                accDataArr[2] = processed_data
        ...
        with open('shared_array.json', 'w') as file:
